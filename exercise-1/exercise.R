...
made.in.1997 <- vehicles[vehicles$year == 1997,]
...
made.in.1997 <- made.in.1997[order(made.in.1997$hwy), ]
...
made.in.1997$average <- (made.in.1997$hwy + made.in.1997$cty) / 2
...
id.worst.hwy <- two.wheel.20.mp$id[two.wheel.20.mpg$hwy == min(two.wheel.20.mpg$hwy)]
...
MakeYearFilter <- function(make, year) {
  matching.make.year <- vehicles[vehicles$make == make & vehicles$year == year, ]
  return(matching.make.year[matching.make.year$hwy == max(matching.make.year$hwy), ])
}
